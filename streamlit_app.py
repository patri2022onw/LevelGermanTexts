...
class GermanNERService:
    """Named Entity Recognition service for German text using NLTK"""
    ...
    def _download_nltk_data(_self):
        ...
        if not NLTK_AVAILABLE:
            logger.warning("NLTK not available, using fallback NER")
            return False
        ...
        try:
            ...
            logger.info("Downloaded NLTK data packages")
            return True
    ...
    def initialize(self):
        ...
        if NLTK_AVAILABLE:
            success = self._download_nltk_data()
            if success:
                self.initialized = True
                logger.info("NLTK NER service initialized successfully")
            else:
                self.fallback_mode = True
                logger.warning("NLTK initialization failed, using fallback NER")
        else:
            self.fallback_mode = True
            logger.warning("NLTK not available, using fallback NER")
            
    def extract_entities(self, text: str) -> Set[str]:
        """Extract named entities from text using NLTK with German enhancements"""
        ...
        try:
            ...
            # Use NLTK's English NER as a starting point
            nltk_entities = self._extract_nltk_entities(text)
            entities.update(nltk_entities)
            
            # Add German-specific entity detection
            german_entities = self._extract_german_entities(text)
    ...
    def _extract_nltk_entities(self, text: str) -> Set[str]:
                ...
                if isinstance(chunk, Tree):
                    ...
                    if entity_type in ['PERSON', 'ORGANIZATION', 'GPE', 'LOCATION']:
                        if len(entity_name.strip()) > 1:
                            entities.add(entity_name.strip().lower())
    ...
    def _extract_german_entities(self, text: str) -> Set[str]:
        """Extract German-specific entities using conservative heuristics"""
        entities = set()
        
        # Only detect very obvious German place names (major cities and countries)
        german_places = {'Berlin', 'München', 'Hamburg', 'Köln', 'Frankfurt', 'Stuttgart', 'Düsseldorf', 'Dortmund',
                        'Essen', 'Leipzig', 'Bremen', 'Dresden', 'Hannover', 'Nürnberg', 'Duisburg', 'Bochum',
                        'Deutschland', 'Österreich', 'Schweiz', 'Bayern', 'Sachsen', 'Hessen', 'Baden'}
        
        # Find capitalized words
        tokens = re.findall(r'\b[A-ZÄÖÜ][a-zäöüß]*\b', text)
        
        for token in tokens:
            # Only include well-known German places
            if token in german_places:
                entities.add(token.lower())
                    
        return entities
    
    def _fallback_ner(self, text: str) -> Set[str]:
        """Conservative fallback NER using minimal heuristics"""
        entities = set()
        
        # Find capitalized words
        tokens = re.findall(r'\b[A-ZÄÖÜ][a-zäöüß]*\b', text)
        
        # Only very obvious German place names and some function words
        german_places = {'Berlin', 'München', 'Hamburg', 'Köln', 'Frankfurt', 'Stuttgart', 'Düsseldorf', 'Dortmund',
                        'Essen', 'Leipzig', 'Bremen', 'Dresden', 'Hannover', 'Nürnberg', 'Duisburg', 'Bochum',
                        'Deutschland', 'Österreich', 'Schweiz', 'Bayern', 'Sachsen', 'Hessen', 'Baden'}
        
        # German articles and function words that should be filtered
        german_function_words = {'Der', 'Die', 'Das', 'Den', 'Dem', 'Des', 'Ein', 'Eine', 'Einen', 'Einem', 'Einer', 'Eines'}
        
        for token in tokens:
            # Skip function words
            if token in german_function_words:
                continue
            
            # Only include well-known places
            if token in german_places:
                entities.add(token.lower())
...
class GermanLanguageAnalyzer:
    """Main class for analyzing German texts based on CEFR levels using simplemma and NLTK NER"""
    ...
    def get_word_level(self, lemma: str) -> str:
        """Get the CEFR level of a word, returning C2 for unknown words"""
        lemma_lower = lemma.lower()
        return self.word_levels.get(lemma_lower, 'C2')
    ...
    def analyze_text(self, text: str, target_level: str) -> Dict:
        """Analyze text and find words above the target level using simplemma and NLTK NER"""
        ...
        debug_info = {
            ...
            'ner_method': 'NLTK' if not self.ner_service.fallback_mode else 'Fallback'
        }
        ...
        for i, token in enumerate(tokens):
            ...
            # Skip named entities using NLTK NER
            ...
            # Check word level (now always returns a level, C2 for unknown words)
            word_level = self.get_word_level(lemma)
            debug_info['word_levels_found'][lemma] = word_level
                
            if self.is_above_level(word_level, target_level):
...
def create_leveled_text(analyzer: GermanLanguageAnalyzer, text: str, target_level: str, 
    ...
    for i, token in enumerate(tokens):
        ...
        if analyzer.is_above_level(word_level, target_level):
...
def get_analyzer() -> GermanLanguageAnalyzer:
    ...
    # Initialize NER service
    with st.spinner("Loading NLTK NER model..."):
...
def main():
    ...
    st.markdown("Adjust German texts based on CEFR levels using simplemma lemmatization and NLTK NER")
    
    # Check NLTK availability
    if not NLTK_AVAILABLE:
        st.warning("⚠️ NLTK not installed. Install with: `pip install nltk`")
        st.info("Using fallback NER (basic capitalization rules)")
    
    # Get cached analyzer
    with st.spinner("Loading vocabulary files and NLTK NER model..."):
    ...
    with st.sidebar:
        ...
        if NLTK_AVAILABLE and not analyzer.ner_service.fallback_mode:
            st.success("✅ NLTK German NER loaded")
            st.caption("Enhanced German-specific entity detection")
        else:
        ...
        with st.expander("📚 Vocabulary Preview"):
            level_filter = st.selectbox("Filter by level:", ["All"] + ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'])
...
und die Vögel zwitschern fröhlich in den Bäumen. Berlin ist eine wunderschöne Stadt."""
    ...
    if st.button("🔍 Analyze Text", type="primary", use_container_width=True):
        if not text_input:
            ...
            with st.spinner("Analyzing text with simplemma and NLTK NER..."):
                try:
                    # Perform analysis
    ...
    - Uses NLTK for named entity recognition with German-specific enhancements
